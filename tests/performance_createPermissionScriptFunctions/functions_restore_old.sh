# these methods have been replaced due to perfomance issues
# I keep them around for testing purposes/etc/comparision.
#
# -> this is the commit in which they have been updated
#commit b9b2af4b29c576ba0b52fa232960995bc5c99838 (HEAD -> master)
#Author: evermind <evermind@tuxfamily.org>
#Date:   Tue May 30 14:48:35 2023 +0200
#     performance: create{Ext}DataPermUpdateScript() now runs much faster



function createDataPermUpdateScriptOld() {
    local workDir="$1"
    local oldGid="$2"
    local newGid="$3"
    local oldUid="$4"
...
function createExtDataPermUpdateScriptOld() {
    local workDir="$1"
    local oldGid="$2"
    local newGid="$3"
